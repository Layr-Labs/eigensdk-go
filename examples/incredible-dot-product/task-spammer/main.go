...
func main() {
	...
	// In this case, the sequence will generate a pair of vectors with values lower than the iteration number in
	// an ascending sequence that increases by 1 per iteration.
	linearSeq := LinearRangeSequence()

	// 4. Build the task spammer, providing the task creator and the task spammer config.
	taskSpammer, err := taskspammer.NewTaskSpammer(logger, taskSpammerConfig, taskCreator, linearSeq)
	if err != nil {
