...
var (
	...
	OperatorNotPartOfTaskQuorumErrorFn = func(operatorId types.BlsOperatorId, taskIndex types.TaskIndex) error {
...
type aggregatedOperators struct {
	...
	signersOperatorIdsSet map[types.BlsOperatorId]bool
}
...
type BlsAggregatorService struct {
	...
	signedTaskRespsCs map[types.TaskIndex]chan types.SignedBlsTaskResponseDigest
...
func NewBlsAggregatorService(avsRegistryService avsregistry.AvsRegistryService, logger logging.Logger) *BlsAggregatorService {
	return &BlsAggregatorService{
		...
		signedTaskRespsCs:    make(map[types.TaskIndex]chan types.SignedBlsTaskResponseDigest),
		taskChansMutex:       sync.RWMutex{},
...
func (a *BlsAggregatorService) InitializeNewTask(
	...
	signedTaskRespsC := make(chan types.SignedBlsTaskResponseDigest)
...
func (a *BlsAggregatorService) ProcessNewSignature(
	...
	case taskC <- types.SignedBlsTaskResponseDigest{
...
func (a *BlsAggregatorService) singleTaskAggregatorGoroutineFunc(
	...
	quorumThresholdPercentages []types.QuorumThresholdPercentage,
	timeToExpiry time.Duration,
	signedTaskRespsC <-chan types.SignedBlsTaskResponseDigest,
) {
		...
		case signedTaskResponseDigest := <-signedTaskRespsC:
			...
			if !ok {
				...
				digestAggregatedOperators = aggregatedOperators{
					...
					signersOperatorIdsSet:      map[types.BlsOperatorId]bool{signedTaskResponseDigest.OperatorId: true},
			...
			if checkIfStakeThresholdsMet(digestAggregatedOperators.signersTotalStakePerQuorum, totalStakePerQuorum, quorumThresholdPercentagesMap) {
				nonSignersOperatorIds := []types.BlsOperatorId{}
...
func (a *BlsAggregatorService) verifySignature(
	taskIndex types.TaskIndex,
	signedTaskResponseDigest types.SignedBlsTaskResponseDigest,
	operatorsAvsStateDict map[types.BlsOperatorId]types.OperatorBlsAvsState,
) error {
...
func getG1PubkeysOfNonSigners(signersOperatorIdsSet map[types.BlsOperatorId]bool, operatorAvsStateDict map[[32]byte]types.OperatorBlsAvsState) []*bls.G1Point {
