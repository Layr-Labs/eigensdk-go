...
type TaskResponder[Input any, Output any] interface {
	...
	RespondToTask(task types.Task[Input], taskResponse types.TaskResponse[Output], nonSignersStakesAndSig types.NonSignerStakesAndSignature) error
	...
	HashTaskResponse(taskResponse types.TaskResponse[Output]) (types.TaskResponseDigest, error)
}
...
type ChallengeRaiser[Input any, Output any] interface {
	...
	RaiseChallenge(task types.Task[Input], taskResponse types.TaskResponse[Output], TaskResponseMetadata types.TaskResponseMetadata, NonSigningOperatorPubKeys []types.BN254G1Point) error
}
