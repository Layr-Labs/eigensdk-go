...
func BuildAll(
	...
	if err != nil {
		return nil, utils.WrapError(errors.New("Failed to create Eth Http client"), err)
	}

	ethWsClient, err := eth.NewClient(config.EthWsUrl)
	if err != nil {
		return nil, utils.WrapError(errors.New("Failed to create Eth WS client"), err)
	}
...
func (config *BuildAllConfig) buildElClients(
	...
	if err != nil {
		return nil, nil, utils.WrapError(errors.New("Failed to create ELChainWriter"), err)
	}
...
func (config *BuildAllConfig) buildAvsClients(
	...
	if err != nil {
		return nil, nil, nil, utils.WrapError(errors.New("Failed to create ELChainSubscriber"), err)
	}
