...
func (t *GeometricTxManager) Start(ctx context.Context) {
	go func() {
		for {
			select {
			case <-ctx.Done():
...
func (t *GeometricTxManager) ProcessTransaction(ctx context.Context, req *TxnRequest) error {
	...
	for retryFromFailure < maxSendTransactionRetry {
		...
		txn, err = t.gasOracle.UpdateGas(ctx, req.Tx, req.Value, gasTipCap, gasFeeCap, from)
		if err != nil {
...
func (t *GeometricTxManager) ReceiptChan() chan *ReceiptOrErr {
